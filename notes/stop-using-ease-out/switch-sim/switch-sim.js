import { SwitchSim } from "/notes/stop-using-ease-out/switch-sim/switch-sim-sim.mjs";

customElements.define(
  "switch-sim",
  class SwitchSimElement extends HTMLElement {
    constructor() {
      super();
      ...
      this.innerHTML = html`
        <div class="switch-sim-render"><canvas></canvas></div>
        ${beforeGraphSlotHTML}
      ...
      this.classList.add("switch-sim");
      if (showGraph) {
        this.classList.add("switch-sim-with-graph");
      }
      ...
      this.renderCanvas = this.querySelector(".switch-sim-render canvas");

      this.graphCanvas = this.showGraph
        ? this.querySelector(".switch-sim-graph canvas")
        : null;

      this.renderCanvas.addEventListener("pointermove", this.onPointerMove);
    ...
    renderSim() {
      ...
      // presser force
      if (
        (this.visibleForces.has("all") || this.visibleForces.has("presser")) &&
        Math.abs(this.sim.presserForce) > 0
      ) {
        drawForceLine(
          cc,
...
function drawForceLine(cc, x, y, x2, y2, headSize, color, text = null) {
  ...
  cc.moveTo(x, y);
  cc.lineTo(x2, y2);
  cc.lineTo(x2 + (-ny * 0.5 - nx) * headSize, y2 + (nx * 0.5 - ny) * headSize);
  cc.moveTo(x2, y2);
  cc.lineTo(x2 + (ny * 0.5 - nx) * headSize, y2 + (-nx * 0.5 - ny) * headSize);
  cc.stroke();
